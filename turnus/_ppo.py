    ...
    def train(self, env, input_dim: tuple, count_of_envs: int, count_of_iterations: int, count_of_steps: int, batch_size: int, count_of_epochs = 4, first_iteration = 0, seed = None):
        ...
        for iteration in range(first_iteration, count_of_iterations):
            ...
            for step in range(count_of_steps):
                ...
                mem_observations[step] = observation.clone()
                ...
                #==== Paralel env =====
                observation, mask, rewards, terminal = env.step(actions)
                ...
                if terminal:
                    ...
                    if(min_obj < best_obj):
                        ...
                        best_observation = {'best_obj': best_obj, 'iteration': iteration, 'episode': len(scores),
                            'observation': list(map(int,observation[indices[0]].tolist()))
                        }
            ...
            with torch.no_grad():
                _, R = self.model(observation.to(self.device))
