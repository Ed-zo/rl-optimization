...
class ScoreLogger(Logger):
    ...
    def log(self, iteration, rewards, infos):
        ...
        if iteration % self.print_every == 0:
            print('iteration', iteration, '\tepisode', episode, '\tavg score', avg_reward if self.score_transformer_fn == None else avg_score, 
                  '\tbest score', self.mva.best_score if self.score_transformer_fn == None else best_score, 
                  '\tbest avg score', self.mva.best_avg_score if self.score_transformer_fn == None else best_avg_score)
